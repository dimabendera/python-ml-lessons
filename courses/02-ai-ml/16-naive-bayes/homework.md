...
Далі зроби класи нерівними штучно: залиш у навчальній вибірці лише 10% обʼєктів
...
Напиши `BernoulliNB` з нуля й зʼясуй, у чому саме він відрізняється від
...
1. Реалізуй клас із методами `навчити(X, y)` і `передбачити_ймовірності(X)`, де `X` —
   ...
   `p[c, j] = (кількість обʼєктів класу c з ознакою j + α) / (обʼєктів класу c + 2α)`.
...
- окремою клітинкою виведено ймовірності `MultinomialNB(alpha=1)` і `BernoulliNB(alpha=1)`
  ...
  пʼятьма словами, яких у тексті **немає**);
...
- **Рівень 2.** Копії колонки зручно робити через `np.hstack([X] + [X[:, [0]]] * скільки)`.
  ...
  Порівнюй моделі на тому самому розбитті: `random_state=0` і `stratify` обовʼязкові.
- **Рівень 3.** Формулу з відсутніми ознаками зручно записати одним виразом:
  ...
  правдоподібності рядка. Жодного циклу по обʼєктах не потрібно.
